...
class WPML_Sticky_Links {
	...
	function _no_wpml_warning() {
		...
		<?php
		printf(
			__( 'WPML Sticky Links is enabled but not effective. It requires <a href="%s">WPML</a> in order to work.', 'wpml-sticky-links' ),
			'https://wpml.org/'
		);
		?>
			</p></div>
	...
	function _old_wpml_warning() {
		...
		<?php
		printf(
			__( 'WPML Sticky Links is enabled but not effective. It is not compatible with  <a href="%s">WPML</a> versions prior 2.0.5.', 'wpml-sticky-links' ),
			'https://wpml.org/'
		);
		?>
			</p></div>
	...
	function icl_dashboard_widget_content() {
		...
		<?php
			echo __(
				'With Sticky Links, WPML can automatically ensure that all links on posts and pages are up-to-date, should their URL change.',
				'wpml-sticky-links'
			);
		?>
				 </p>

		<p><a class="button secondary" href="<?php echo 'admin.php?page=wpml-sticky-links'; ?>">
														<?php
														echo __( 'Configure Sticky Links', 'wpml-sticky-links' )
														?>
			</a></p>
