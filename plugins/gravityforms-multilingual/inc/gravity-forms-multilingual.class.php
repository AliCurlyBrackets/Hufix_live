...
abstract class Gravity_Forms_Multilingual {
	...
	public function __construct() {
		...
		add_filter( 'gform_form_pre_process_async_task', [ $this, 'gform_pre_process_async' ], 10, 2 );
	...
	public function get_document_view_link( $post_view_link, $label, $current_document, $element_type, $content_type ) {
		if ( 'package' === $element_type && self::SLUG === $content_type ) {
	...
	private function handle_multi_input( $field, $st_context, $notificationRouting ) {
		...
		if ( is_array( $field->choices ) ) {
			foreach ( $field->choices as $index => $choice ) {
				...
				if ( isset( $choice['file_url'], $choice['attachment_id'] ) ) {
					$attachment_id = (int) $choice['attachment_id'];
					$translated_id = (int) apply_filters( 'wpml_media_id', $attachment_id );
					if ( $translated_id !== $attachment_id ) {
						$field->choices[ $index ]['file_url']      = apply_filters( 'wpml_media_url', $choice['file_url'] );
						$field->choices[ $index ]['attachment_id'] = $translated_id;
					}
				}
			}
		}
	...
	/**
	 * Translate the form in the async notification background processor context.
	 *
	 * GF 2.10+ processes notifications asynchronously via a loopback HTTP request. In that context
	 * GFAPI::get_form() bypasses gform_pre_process, so routing condition values stay in the default
	 * language and never match the translated entry values. This hook replicates the translation step.
	 *
	 * @param array $form
	 * @param array $_entry
	 *
	 * @return array
	 */
	public function gform_pre_process_async( $form, $_entry ) {
		return $this->translate( $form );
	}
	...
	public function gform_pre_replace_merge_tags( $text, $form ) {
		if ( isset( $form['confirmation'] ) && rgar( $form['confirmation'], 'message' ) === $text ) {
			...
			if ( $text === $confirmation_message ) {
				...
				if ( isset( $form['confirmation']['event'] ) && 'form_save_email_sent' === $form['confirmation']['event'] ) {
