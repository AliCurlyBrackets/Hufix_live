...
if ( ! function_exists( 'toolset_common_initialize' ) ) {
    ...
    function toolset_common_initialize( $path, $url ) {
        ...
        // Resolve symlinks for reliable comparison in development environments.
        $real_path = realpath( $path );

        // Save the url in the matching path
        foreach ( $toolset_common_paths as $key => $data ) {
            $stored_real = realpath( $toolset_common_paths[ $key ]['path'] );
            if (
                $toolset_common_paths[ $key ]['path'] == $path
                || ( false !== $real_path && false !== $stored_real && $real_path === $stored_real )
            ) {
