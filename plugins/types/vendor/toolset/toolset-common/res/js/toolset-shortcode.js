...
Toolset.shortcodeManager = function( $ ) {
    ...
    self.init = function() {
        ...
        try {
            ...
            // Ensure minimal structure exists so downstream code like Types initTemplates can safely access self.templates.attributes.
            self.templates = self.templates || {};
            self.templates.attributes = self.templates.attributes || {};
            if ( window.TOOLSET_DEBUG ) {
                console.warn( 'Toolset shortcodeManager.initTemplates skipped:', e.message );
            }
        }
        ...
        try {
            ...
            // initDialogs may fail when dialog DOM elements are not present in the block editor or Site Editor context.
            if ( window.TOOLSET_DEBUG ) {
                console.warn( 'Toolset shortcodeManager.initDialogs skipped:', e.message );
            }
        }
        ...
        try {
            ...
            // initInputButton may fail when the toolbar is not present.
            if ( window.TOOLSET_DEBUG ) {
                console.warn( 'Toolset shortcodeManager.initInputButton skipped:', e.message );
            }
        }
    };
